...
if (isset($_POST['old'])) {
    ...
    foreach($user_records as $row) {
      ...
      $db_pw1 = $row['password']?$row['password']:substr(str_shuffle("123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"),0,15);

      if (password_verify($old, $db_pw2) OR password_verify($old, $db_pw1)) {
	echo "SUCCESS";
        $update = exec_sql("update users set password=?, password2=? WHERE id = ?",array($new_pw,$new_pw,$user_id),"new password",2);
