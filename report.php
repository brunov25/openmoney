...
if ($details) {
  ...
  $last_transactions = exec_sql("select * from (select * from user_journal order by id DESC) AS x where currency=? GROUP BY trading_account; ",array($currency),"items in journal",0);
}
$data = '';
$total = 0.00;
$total2 = 0.00;
