...
/**
 * The obligitory initialData World example
 *
 ...
 *
 * @uri /initialData
 * @uri /initialData/:name
 */
class initialData extends Resource
{
    /**
     ...
     * @return str
     */
    public function sayinitialData($name = 'World')
    {
        return 'initialData '.htmlspecialchars(ucwords($name));
    }

    /**
     * Respond with initialData in French.
     *
     * @method GET
     * @lang fr
     * @param  str $name
     * @return str
     */
    public function sayinitialDataInFrench($name = 'Monde')
    {
        return 'Bonjour '.htmlspecialchars(ucwords($name));
    }

    /**
     * The @priority annotation makes this method take priority over {@link sayinitialData}.
     *
     ...
     * header or URL extension (ie: /initialData.json) and causes the response to automatically
     ...
     */
    public function sayinitialDataComputer()
    {
     ...
     *
     * curl -i -H "Content-Type: application/json" -X POST -d '{"initialData": "computer"}' http://localhost/www/tonic/web/initialData.json
     *
     * @method POST
