	...
	public function sayaccess()
	{
			...
			$result = new Response(200, array(
					'profile' => array('id' => intval($this->user['id']),
							...
							'customValues' => array(array("internalName" => "gender",
											 			  "fieldId" => 2,
											 			  "displayName" => "Gender",
											 			  "value" => "Male",
											 			  "possibleValueId" => 1),
													array("internalName" => "address",
											 			  "fieldId" => 3,
											 			  "displayName" => "Address",
...
class accessRegister extends Resource
{
	...
	public function sayaccessRegister()
	{
			...
			function email_letter($to='deefactorial@gmail.com',$from='noreply@openmoney.org',$subject='no subject',$msg='no msg') {
				$headers  = "From: $from,\r\n";
			...
			$namePatternNumberOfCharacters = "/^['-\p{L}]{3,32}$/u"; // 3-32 international letters
			...
			if( $error == '' ) {
				...
				$user_q = mysqli_query($db, $test = "INSERT INTO users (`fname`, `lname`, `user_name`, `email`, `password`, `password2`, `init_space`, `init_curr`, `confirmed`) VALUES ('$firstname', '$lastname', '$username', '$email', '$password_hash', '$password_hash', '$init_space', '$init_currency', '$confirmed')") or die($test . mysqli_error($db));
			...
			if ($error == '') {
				...
				$result = new Response(200, array(
						'profile' => array('id' => intval($userID),
